...
tokenize.df = function(df) {
  ...
  for (i in 1:length(df$Description)) {
    ...
    temp = bind_rows(temp, as.vector(tokens))
...
tf_idf = function(split_df, large_scale = T) {
  ...
  if(large_scale == T){
    ...
    for (i in 1:length(split_df)) {
      ...
      temp = apply(split_df[[i]][,6:length(split_df[[i]])], 2, sum)
      #getting some sort of normalized data for the master list
      split_df[[i]] = bind_rows(split_df[[i]], temp) 
      split_df[[i]] = bind_rows(split_df[[i]], colSums(split_df[[i]] > 0))
      split_df[[i]] = bind_rows(split_df[[i]], log10((nrow(split[[i]])-1) / split_df[[i]][nrow(split_df[[i]]),]))
