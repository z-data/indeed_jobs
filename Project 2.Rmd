...
values = stem_or_non(sarah_tokens, stem_df, non_stem_df)
```
...
sampling = function(resume_is_stem) {
  ...
  if(resume_is_stem == FALSE){
    ...
    for (index in 1:length(sample_job_descriptions)) {
      ...
      if(user_input == "y"){
        ...
        sample_job_descriptions$liked_or_not = 0
      }
    } return(sample_job_descriptions)
    #repeat process for stem
  }else{
    sample_job_descriptions <- sample_n(stem_jobs_df, 5)
    sample_job_descriptions$liked_or_not <- NULL
    for (index in 1:length(sample_job_descriptions)) {
      job_desc <- str(sample_job_descriptions$Description[index])
      user_input <- readline(prompt= job_desc)
...
sample_job_descriptions = sampling(resume_is_stem)

tokenize.df(sample_job_descriptions)
...
if(resume_is_stem == FALSE){
  ...
  #change from vector -> df
  sample_job_descriptions <- as.data.frame(sample_job_descriptions)
...
#function to tokenize the job descriptions
tokenize_job_desc = function(path) {
  #inputting sarah resume 
  resume = path
  resume = str_c(resume, sep = " ", collapse = " ") 
...
#vector to hold the tokens extracted from the job descriptions
params <- c()


sample_job_descriptions$tokens <- NULL

#iterate thru the sampled job descriptions, tokenize them, and add tokens to the vector

df_for_model <- data.frame()

  for(row in length(sample_job_descriptions)){
    param_row <- tokenize_job_desc(sample_job_descriptions$Description[index])
    params <- c(params, param_row)
