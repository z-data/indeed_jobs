...
```
```{r}
#this gets a total sum of the proportion of all the words 
master_li = list()

for (i in 1:length(split_df)) {
  temp = apply(split_df[[i]][,6:length(split_df[[i]])], 2, sum)
  master_li[[names(split_df)[i]]] = temp / nrow(split_df[[i]])
}
...
```{r}
#setting the resumes up as tokens 
...
zac1 = tokenize_resume("test1.txt")
zac2 = tokenize_resume("test2.txt")
...
```{r}
#tf_idf_df is the DF with job totals 
# split_df_freqs is the DF with tf-idf for each job description
# master_li has the total proportion of words in each job field 
sort(master_li$human_resource_specialist, decreasing = T)
...
best_matches_overview_df = function(li, resume) {
  matches = matrix(NA, nrow = nrow(li), ncol = 1)
  ...
  rownames(matches) = rownames(li)
  return(matches)
}


bm_new = best_matches_overview(master_li, zac2)
print(sort(bm_new[,1], decreasing = T))

bm_idf = best_matches_overview_df(tf_idf_df, zac2)
print(sort(bm_idf[,1], decreasing = T))
#converting job names to a matrix
job_names  = names(sort(bm_new[,1], decreasing = T)[1:5])
job_names = str_split_fixed(job_names, "", 1)


job_names1  = names(sort(bm_idf[,1], decreasing = T)[1:5])
job_names1 = str_split_fixed(job_names1, "", 1)
```
...
```{r}
matching_descriptions = split_df_freqs
for(i in 1:length(matching_descriptions)) {
  matching_descriptions[[i]]$descriptions = NA
  for(j in 1:length(matching_descriptions[[i]])) {
...
```{r}
best_matches = function(df, job_name, resume, split_df) {
  matches = list()
  ...
  for (i in 1:nrow(job_name)) {
    ...
    matches[[job_name[i,1]]] = split_df[[job_name[i,1]]][index, 4]
    print(index)
    }
  return(matches)
}
...
best = best_matches(split_df_freqs, job_names1, zac2, split_df)
best


```
```{r}
split_df_freqs$ux_designer
```
...
resume_string <- read_file("/Users/nikhilgopal/Downloads/resume.txt")
