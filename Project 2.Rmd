...
```{r}
naive_method = function(split_df, scale_large = T) {
  #getting the broad scale version of the naive method 
   if(scale_large == T) {
    ...
    for (i in 1:length(split_df)) {
      ...
      factor = mean(apply(split_df[[i]][,6:length(split_df[[i]])], 1, function(x) sum(x > 0)))
    ...
    for (i in 1:length(split_df)) {
      print(i)
      for (j in 1:nrow(split_df[[i]]))
      #finding the frequency of words 
      split_df[[i]][j,6:ncol(split_df[[i]][j,])] = split_df[[i]][j, 6:ncol(split_df[[i]][j,])] / length(6:ncol(split_df[[i]][j,]))
    }
  }
  return(split_df)
}

master_li = naive_method(split_df)
freq_split_df = naive_method(split_df, F)


for(i in 1:length(freq_split_df)) {
  print(i)
  for(j in 6:nrow(freq_split_df[[i]])) {
    freq_split_df[[i]][,j] = as.numeric(freq_split_df[[i]][,j])
  }
}

rowSums(split_df[[1]][,6:length(split_df[[1]])])
```
...
tf_idf = function(split_df, large_scale = T) {
  ...
  if(large_scale == T){
    ...
    for (i in 1:length(split_df)) {
      ...
      #getting some sort of normalized data for the master list
      send_back = bind_rows(send_back, temp) 
      print("HERE")
      send_back = bind_rows(send_back, colSums(split_df[[i]][,6:length(split_df[[i]])] > 0))
      send_back = bind_rows(send_back, log10((nrow(split_df[[i]])-1) / split_df[[i]][nrow(split_df[[i]]),6:length(split_df[[i]])]))

      for (j in 1:nrow(split_df[[i]])) {
        inverse = split_df[[i]][nrow(split_df[[i]]),6:ncol(split_df[[i]])]
        freqs = split_df[[i]][j,6:ncol(split_df[[i]])]
        tf_idf = freqs * inverse
        split_df[[i]][j,6:ncol(split_df[[i]])] = tf_idf
      }
    }
    return(split_df)
  }
...
tfIdf_split_df = tf_idf(freq_split_df, F)


apply(split_df[[1]][,6:length(split_df[[1]])], 2, sum)


tfIdf_split_df$business_analyst
freq_split_df$business_analyst

'''
#checking if the 2 methods look similar 
sort(tf_idf_df[3,], decreasing = T)
sort(master_li$data_scientist, decreasing = T)


idf_list = list()
for (i in 1:nrow(tf_idf_df)) {
  idf_list[[rownames(tf_idf_df)[i]]] = tf_idf_df[i,]
}
